...
### 풀이
* 어떤 수 a의 배수는 항상 a를 약수로 가진다(idea)
* f(i) = i의 약수의 합을 가지는 배열
* g(i) = i 이하의 자연수들의 약수 누적합, 즉 f(1) + ... + f(i) = g(i-1) + f(i)

### 참고
* input()으로 받으면 시간초과 -> sys.stdin.readline()
* 미리 배열에 값을 저장해놓는게 더 빠르다!!! 매번 계산하는 것보다
